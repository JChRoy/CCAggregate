...
def _coerce_weight_matrix(w):
    if w is None:
        return None
    w = np.asarray(w, dtype=float)
    if w.ndim == 1:
        w = w[:, None]
    return w if w.ndim == 2 and w.size else None
...
def parallel_best_param_ensemble_multidim(
    ...
    Notes: 
      Canonical solutions are sign- and order-indeterminate, so weights are
      aligned across splits before averaging. Components are matched to the
      first split by Hungarian assignment on the absolute weight correlations
      of **view 0**, and the sign of each component is then set so that its
      view-0 weight vector correlates positively with the reference. All other
      views inherit that ordering and sign.
      This assumes view 0 is stable enough across splits to serve as a
      reference. Where view 0 is sparse, low-dimensional or weakly determined,
      matching can be arbitrary and the averaged weights will be attenuated
      toward zero rather than obviously wrong. ``alignment_corr`` in the
      returned dict gives the view-0 correlation with the reference for each
      split and component; values near zero mean the alignment for that split
      is not trustworthy. If they are widely spread, pass a different view
      order so that the best-determined view is first, or interpret only the
      per-split results.        
    ...
    if weight_lists_per_split:
        ...
        alignment_corr = np.full((S, K), np.nan)   # diagnostics
        ...
        for s in range(S):
            ...
            corr_block = np.nan_to_num(np.abs(corr_block), nan=0.0,
            ...
            for j in range(K):
                ...
                if not np.isfinite(r):
                    r = 1.0
                alignment_corr[s, j] = r          # keep the signed value
        ...
        # warn if the reference view does not align reliably
        med_abs = np.nanmedian(np.abs(alignment_corr), axis=0)
        if np.any(med_abs < 0.3):
            warnings.warn(
                f"Weak alignment on view 0: median |r| per component = "
                f"{np.round(med_abs, 2)}. Averaged weights may be attenuated. "
                "See alignment_corr in the returned dict."
            )
    ...
    final_results = {
        ...
        "alignment_corr": alignment_corr if weight_lists_per_split else None,
        "weights_per_split": [res.get("weights") for res in results_list],
...
def run_rcca_stability_selection_multidim(
    ...
    for k in range(K):
        ...
        def _run_single_split(split_idx, _k=k, _meta_k=filtered_metadata_k, _kept_views=kept_view_indices_dim_k):
            ...
            kept_indices = [_kept_views[i] for i in kept_loc_indices]
