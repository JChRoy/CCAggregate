...
from .utils import get_fold_data, train_test_split_by_group
...
def permute_score_significance_multidim(
    ...
    * Each permutation uses an independent seed derived from the split seed,
    ...
    # ------------------------------------------------------------------
    _nan_K   = lambda: np.full(K, np.nan)
    _nan_K2  = lambda: np.full((K, 2), np.nan)
...
def permute_rgcca_significance_multidim(
    ...
    * Cells for dimensions with no selected features are filled with ``np.nan``.
    ...
    for k in range(K):
        ...
        def _run_one_permutation(perm_seed: int, split_idx: int, _vm_k=view_metadata_k, _k=k):
            """
            ...
            N = _vm_k[response_view]["data"].shape[0]
            perm_order = local_rng.permutation(N)

            permuted_metadata_k = []
            for v_idx, meta in enumerate(_vm_k):
            ...
            try:
                ...
                if not other_view_indices:
                    warnings.warn(
                        f"Split {split_idx}, Dim {_k + 1}: only one view present; "
                ...
                warnings.warn(
                    f"Permutation failed — Split {split_idx}, Dim {_k + 1}: {exc}. "
