...
def get_fold_data(
    ...
    * Gaussian Copula transform is applied independently in the train and
      test set for distribution enforcement, but not learning. 
    """
    train_views = []
    test_views = []
...
def extract_fold_scores(cv_aux):
    if cv_aux is None:
        return None
...
def select_best_idx_from_cv(
    cv_records,
    param_list,
    selection_rule="max_mean",
    param_complexity_fn=None,
):
    means = np.asarray([rec["mean_cv"] for rec in cv_records], dtype=float)
    ...
    ses = np.asarray([rec["se_cv"] for rec in cv_records], dtype=float)
    ...
    candidate_idxs = np.flatnonzero(np.isfinite(means) & (means >= cutoff))
